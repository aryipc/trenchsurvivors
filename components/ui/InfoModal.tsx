...
const Section: React.FC<{title: string; children: React.ReactNode}> = ({ title, children }) => (
    <div className="mb-8">
        <h2 className="text-3xl text-green-300 mb-4 border-b-2 border-gray-600 pb-2">{title}</h2>
        ...
        <div className="fixed inset-0 bg-black/80 flex justify-center items-center z-50" onClick={onClose}>
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-4xl text-green-400">Whitepaper</h1>
                ...
                <Section title="Market Threats">
                     {Object.keys(ENEMY_DATA).map(key => {
                        ...
                        switch(data.name) {
                            ...
                            case 'Migrating Boss': description = 'A destabilizing market force. While active, your market cap growth is halted until the boss is defeated.'; break;
                        }
                ...
                <Section title="Special Items">
                    {Object.values(ItemType)
                        .filter(type => type !== ItemType.BONKAura && type !== ItemType.DevLock)
                        .map(type => {
                        return (
                            <InfoItem key={type} icon={ICONS[type]} name={type}>
                    ...
                    <InfoItem key={ItemType.DevLock} icon={ICONS[ItemType.DevLock]} name={ItemType.DevLock}>
                        A rare utility item. Freezes all enemies on the screen for 5 seconds upon pickup.
                    </InfoItem>
                </Section>
            </div>
        </div>
